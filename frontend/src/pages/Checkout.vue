        ...
        <div class="checkout-form-container">
            <form id="checkoutForm" @submit="handleSubmit" novalidate autocomplete="off">
                <div class="checkout-heading">
                    <h3>Bước tiếp theo để hoàn thành<span>Tổng cộng</span></h3>
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <input type="text" name="coAddress" id="coAddress" placeholder="Cần Thơ, Việt Nam"
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            ...
                            <input type="radio" name="payment" value="card" id="paymentCard"
                                v-model="checkoutObj.paymentMethod" /><span>Thẻ tín dụng</span>
                        </div>
                    ...
                    <div v-if="checkoutObj.paymentMethod == 'card'">
                        <div class="form-group">
                            <input type="text" name="coCardNum" placeholder="Nhập số thẻ của bạn" id="coCardNum"
                        ...
                        <div class="form-group">
                            <input v-upcase type="text" name="coCardName" placeholder="Nhập tên trong thẻ của bạn "
                        ...
                        <div class="form-group">
                            <div class="form-control">
                                <span
                                    style="font-size: 1.6rem; position: absolute; margin-left: -5px; margin-top: -11px;">Hết hạn:
                                </span>
                                <input
                ...
                <div v-if="user" class="form-group">
                    <input type="submit" value="Xác nhận & Thanh toán" class="btn"
                        :disabled="filterFlowers.length ? false : true" />
                </div>
            </form>
        </div>
    </div>
    ...
    methods: {
        ...
        calculateSummaryPrice: function () {
            ...
            while (i < this.itemQuantity.length) {
                subtotal = subtotal + parseInt(this.filterFlowers[i].flower_price) * this.itemQuantity[i]
                discount = discount + parseInt(this.filterFlowers[i].flower_discount) * this.itemQuantity[i]
                i = i + 1
            }
            if (!this.filterFlowers.length) {
                delivery = 0
            }
        ...
        async getAllCartItem() {
            if (this.user) {
                ...
                existItem.data.forEach(element => {
                    this.cartItem.push(element.flower_id);
        ...
        checkForm: function () {
            ...
            if (!this.checkoutObj.phone) {
                this.errorObj.phoneErr.push('Yêu cầu nhập số điện thoại');
            }
            else {
                if (!this.checkoutObj.phone.startsWith('84')) {
                    this.errorObj.phoneErr.push('Số điện thoại phải bắt đầu 84');
                }

                if (this.checkoutObj.phone.length != 11) {
                    this.errorObj.phoneErr.push('Số điện thoại phải đúng 11 số');
                }

                if (!/[0-9]{11}/.test(this.checkoutObj.phone)) {
                    this.errorObj.phoneErr.push('Số điện thoại chỉ chứa số từ 0-9');
                }
            }

            // Address validate
            if (!this.checkoutObj.address) {
                this.errorObj.addressErr.push('Yêu cầu nhập địa chỉ');
            }

            // Card validate
            if (!this.checkoutObj.paymentMethod) {
                this.errorObj.payErr.push('Yêu cầu chọn phương thức thanh toán');
            }
            else if (this.checkoutObj.paymentMethod == "card") {
                if (!this.cardObj.number) {
                    this.errorObj.numErr.push('Yêu cầu nhập số thẻ');
                }
                else {
                    if (!this.cardObj.number.startsWith('4')) {
                        this.errorObj.numErr.push('Số thẻ visa phải bắt đầu với 4');
                    }

                    if (this.cardObj.number.length != 16) {
                        this.errorObj.numErr.push('Số thẻ visa phải chính xác 16 số');
                    }

                    if (!/[0-9]{16}/.test(this.cardObj.number)) {
                        this.errorObj.numErr.push('Số thẻ visa chỉ chứa số 0-9');
                    }
                }

                if (!this.cardObj.name) {
                    this.errorObj.nameErr.push('Yêu cầu nhập tên');
                }
                else {
                ...
                if (!this.cardObj.expiryDate) {
                    this.errorObj.exDateErr.push('Yêu cầu nhập ngày hết hạn');
                }


                if (!this.cardObj.cvv) {
                    this.errorObj.cvvErr.push('Yêu cầu nhập mã cvv');
                }
                else {
                    if (this.cardObj.cvv.length != 3) {
                        this.errorObj.cvvErr.push('Mã cvv chỉ chứa 3 ký tự');
                    }

                    if (!/[0-9]{3}/.test(this.cardObj.cvv)) {
                        this.errorObj.cvvErr.push('Mã cvv chỉ chứa số');
                    }
                }
        ...
        async sendBillDetails(billId, flowerId, qty) {
            let billDetails = {
        ...
        async handleSubmit(e) {
            ...
            if (!this.checkEmptyErr()) {
                ...
                this.cartItem.forEach((flowerId, index) => {
                    this.sendBillDetails(billId, flowerId, this.itemQuantity[index])
                });
