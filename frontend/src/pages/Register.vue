        ...
        <div class="register-form-container">
            <form id="registerForm" @submit="handleSubmit" novalidate autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="uPassConfirm" placeholder="Xác nhận lại mật khẩu" id="uPassConfirm"
                ...
                <div class="form-group">
                    ...
                    <p>có một tài khoản? <router-link @click="scrollToTop()" to="/login">Đăng nhập</router-link>
                    </p>
                </div>
            </form>
        </div>
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.registerObj.name) {
                this.errorObj.nameErr.push("Yêu cầu nhập tên của bạn");
            }
            else {
                if (!/^[A-Za-z]+$/.test(this.registerObj.name.replace(/\s/g, ""))) {
                    this.errorObj.nameErr.push('Tên chỉ có thể chứa chữ');
                }
            }

            // Email validate
            if (!this.registerObj.email) {
                this.errorObj.emailErr.push("Yêu cầu nhập email");
            }
            else {
                if (!/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$/.test(this.registerObj.email)) {
                    this.errorObj.emailErr.push('Email phải có giá trị');
                }
            }

            // Pass validate
            if (!this.registerObj.pass) {
                this.errorObj.passErr.push('Yêu cầu nhập mật khẩu');
            }
            else {
                ...
                if (this.registerObj.pass.length < 8) {
                    this.errorObj.passErr.push('Mật khẩu chứa ít nhất 8 ký tự');
                }
            }

            // Confirm Pass validate
            if (!this.registerObj.confirm) {
                this.errorObj.confirmErr.push('Yêu cầu xác nhận mật khẩu');
            }
            else {
                if (this.registerObj.pass !== this.registerObj.confirm) {
                    this.errorObj.confirmErr.push('Mật khẩu xác nhận phải phù hợp với mật khẩu');
                }
            }
            ...
            if (!this.registerObj.phone) {
                ...
                if (!this.registerObj.phone.startsWith('84')) {
                    this.errorObj.phoneErr.push('Số điện thoại phảo bắt đầu với 84');
                }
            ...
            if (!this.registerObj.birth) {
                this.errorObj.birthErr.push("Yêu cầu nhập ngày sinh của bạn");
            }
            else {
                ...
                if (dateInput === "Invalid Date") {
                    this.errorObj.birthErr.push("Ngày nhập không hợp lệ");
                }

                if (dateInput.getTime() < dateMin.getTime() || dateInput.getTime() > dateMax.getTime()) {
                    this.errorObj.birthErr.push("Phạm vi sinh nhật có sẵn là từ 150 năm qua đến nay");
                }
            }

            // Gender validate
            if (!this.registerObj.gender) {
                this.errorObj.genderErr.push("Vui lòng chọn 1 giới tính");
            }
        },
